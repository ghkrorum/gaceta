...
function display_cff($atts, $content = null) {
    ...
    array(
        ...
        'exclude' => '',
        'disablelightbox' => $options[ 'cff_disable_lightbox' ],
        ...
        'credit' => $options[ 'cff_show_credit' ],
        'nofollow' => 'true',
        ...
        //Translate
        'seemoretext' => stripslashes( esc_attr( $options[ 'cff_see_more_text' ] ) ),
        'seelesstext' => stripslashes( esc_attr( $options[ 'cff_see_less_text' ] ) ),
        'maptext' => stripslashes( esc_attr( $options[ 'cff_map_text' ] ) ),
        'facebooklinktext' => stripslashes( esc_attr( $options[ 'cff_facebook_link_text' ] ) ),
        'sharelinktext' => stripslashes( esc_attr( $options[ 'cff_facebook_share_text' ] ) ),
        'showfacebooklink' => $options[ 'cff_show_facebook_link' ],
        'showsharelink' => $options[ 'cff_show_facebook_share' ],

        'previouscommentstext' => stripslashes( esc_attr( $options[ 'cff_translate_view_previous_comments_text' ] ) ),
        'commentonfacebooktext' => stripslashes( esc_attr( $options[ 'cff_translate_comment_on_facebook_text' ] ) ),
        'photostext' => stripslashes( esc_attr( $options[ 'cff_translate_photos_text' ] ) ),
        'likesthistext' => stripslashes( esc_attr( $options[ 'cff_translate_likes_this_text' ] ) ),
        'likethistext' => stripslashes( esc_attr( $options[ 'cff_translate_like_this_text' ] ) ),
        'andtext' => stripslashes( esc_attr( $options[ 'cff_translate_and_text' ] ) ),
        'othertext' => stripslashes( esc_attr( $options[ 'cff_translate_other_text' ] ) ),
        'otherstext' => stripslashes( esc_attr( $options[ 'cff_translate_others_text' ] ) ),
    ...
    //If Access Token is blank or 'Use own Access Token' setting is unchecked, or displaying events only, photos only or featured post then use our tokens
    if ($access_token == '' || !$cff_show_access_token || ($cff_events_only && !$cff_is_group) || $cff_photos_only || ($cff_featured_post_active && !empty($atts['featuredpost'])) ) $access_token = $access_token_array[rand(0, 4)];
    ...
    foreach ( $page_ids as $page_id ) {
        ...
        if ($cff_events_only && $cff_events_source == 'eventspage'){
            ...
            foreach ($event_data->data as $event )
            {
                ...
                //Picture
                if($cff_show_media) $cff_post_item .= '<a title="' . $cff_facebook_link_text . '" class="cff-photo" href="'.$event_link.'" '.$target.$cff_nofollow.'><img src="'. $pic_big .'" border="0" /></a>';
                //Start text wrapper
                if ( ($cff_thumb_layout || $cff_half_layout) ) $cff_post_item .= '<div class="cff-details">';
                    ...
                    if ($cff_show_event_details){
                        ...
                        if (!empty($description)){
                            ...
                            $cff_post_item .= '<p class="cff-desc" ' . $cff_event_details_styles . '>' . cff_autolink($description, $link_color=$cff_event_link_color) . '</p>';
                        }
                    }
                //End details
                ...
                if($cff_show_link) $cff_post_item .= '<a class="cff-viewpost" href="' . $event_link . '" ' . $target . $cff_nofollow.' ' . $cff_link_styles . '>'.$cff_facebook_link_text.'</a>';
                // if ( $ticket_uri ) $cff_post_item .= '<a class="cff-viewpost" href="' . $ticket_uri . '" ' . $target . $cff_nofollow.' ' . $cff_link_styles . '>'.$cff_buy_tickets_text.'</a>';
        ...
        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
            ...
            if($cff_photos_only){
                ...
                if($cff_is_group){
                    ...
                    //For pages
                    $cff_posts_json_url = "https://graph.facebook.com/fql?q=SELECT%20pid,created,src_big,link,caption%20FROM%20photo%20WHERE%20pid%20IN%20(SELECT%20pid%20FROM%20photo%20WHERE%20owner='".$u_id."')%20LIMIT%20".$cff_post_limit;
                }
            }
            ...
            if ($cff_cache_time != 0){
                // Get any existing copy of our transient data
                $transient_name = 'cff_' . $page_id . '_' . strtotime($atts['from']) . strtotime($atts['until']) . $atts['featuredpost'] . $cff_albums_only . $cff_albums_source . $cff_photos_only . $cff_post_limit;
            ...
            if( empty($FBdata->data) && (!$cff_featured_post_active || empty($atts['featuredpost'])) && !$cff_ext_multifeed_active ) {
                ...
                if ( $cff_show_post ) {
                    ...
                    if( !$cff_photos_only && !($cff_albums_only && $cff_albums_source == 'photospage') && empty($cff_album_id) ){
                        ...
                        if ($news->type == 'video'){
                            ...
                            if( isset($news->name) && !$cff_is_video_embed ){
                                if (!$cff_title_link) $cff_post_text .= '<a href="'.$link.'" '.$target.$cff_nofollow.' style="color: #'.$cff_posttext_link_color.'">';
                        ...
                        if ($cff_post_type == 'link') {
                            ...
                            if ( isset($news->picture) ){
                                ...
                                if ($cff_link_image && $cff_show_media) {
                                    $cff_shared_link .= '<a class="cff-link" href="'.$link.'" '.$target.$cff_nofollow.'>';
                        ...
                        if ($cff_show_event_title || $cff_show_event_details) {
                            ...
                            if ($cff_post_type == 'event') {
                                ...
                                $event_json_url = 'https://graph.facebook.com/'.$eventID.'?fields=description,location,name,owner,start_time,timezone,venue,id,likes,comments&access_token=' . $access_token . $cff_ssl;
                                ...
                                //Picture
                                if($cff_show_media) $cff_event .= '<a title="'.$cff_facebook_link_text.'" class="cff-event-thumb" href="'.$link.'" '.$target.$cff_nofollow.'><img border="0" src="https://graph.facebook.com/'.$eventID.'/picture?width=200&height=200" /></a>';
                        ...
                        if ($news->type == 'swf') {
                            ...
                            if($cff_media_position == 'above') $cff_media .= ' cff-media-above';
                            $cff_media .= '" ' . $target . $cff_nofollow.'><img src="' . $picture . '" border="0" /></a>';
                        }
                        ...
                        if ($news->type == 'video') {
                            ...
                            if($youtube || $youtu || $youtubeembed) {
                                ...
                                if($cff_video_action == 'facebook') $cff_media_video .= '<a href="http://facebook.com/'.$news->id.'" target="_blank" class="cff-media-overlay"></a>';
                            ...
                            //Else link to the video file
                                ...
                                $cff_media_video = '';
                                if ( ($cff_video_action !== 'facebook' && $cff_mp4_check) || !$cff_disable_lightbox ) $cff_media_video .= '<div class="cff-html5-video"><a href="http://facebook.com/'.$news->id.'" class="cff-html5-play"><i class="fa fa-play cff-playbtn"></i></a><video src="'.$vid_link.'" poster="'.$picture.'" >';

                                $cff_media_video .= '<a title="' . $vid_title . '" class="cff-vidLink" href="' . $link . '" '.$target.$cff_nofollow.'><i class="fa fa-play cff-playbtn"></i><img class="cff-poster" src="' . $picture . '" alt="' . $vid_title . '" /></a>';
                        ...
                        if (!empty($news->comments->data)){
                            ...
                            foreach ($news->comments->data as $comment_item ) {
                                ...
                                $cff_comments .= '"><a href="https://facebook.com/'. $comment_item->from->id .'" class="cff-name" '.$target.$cff_nofollow.' ' . $cff_meta_link_color . '>' . $comment_item->from->name . '</a>' . cff_autolink( $comment, $link_color=str_replace('#', '', $atts['sociallinkcolor']) );
                    ...
                    if($cff_albums_only && $cff_albums_source == 'photospage'){
                        ...
                        if ( $cff_exclude_string != '' ){
                            ...
                            $string_in_post_text = false;
                            $string_in_desc = false;
                            if ( cff_stripos_arr($post_text, $cff_exclude_strings_array) !== false ) $cff_show_post = false;
                        }
...
function cff_getdate($original, $date_format, $custom_date) {
    ...
    switch ($date_format) {
        ...
        default:
            ...
            $cff_second = stripslashes( esc_attr( $options['cff_translate_second'] ) );
            if ( empty($cff_second) ) $cff_second = 'second';

            $cff_seconds = stripslashes( esc_attr( $options['cff_translate_seconds'] ) );
            if ( empty($cff_seconds) ) $cff_seconds = 'seconds';

            $cff_minute = stripslashes( esc_attr( $options['cff_translate_minute'] ) );
            if ( empty($cff_minute) ) $cff_minute = 'minute';

            $cff_minutes = stripslashes( esc_attr( $options['cff_translate_minutes'] ) );
            if ( empty($cff_minutes) ) $cff_minutes = 'minutes';

            $cff_hour = stripslashes( esc_attr( $options['cff_translate_hour'] ) );
            if ( empty($cff_hour) ) $cff_hour = 'hour';

            $cff_hours = stripslashes( esc_attr( $options['cff_translate_hours'] ) );
            if ( empty($cff_hours) ) $cff_hours = 'hours';

            $cff_day = stripslashes( esc_attr( $options['cff_translate_day'] ) );
            if ( empty($cff_day) ) $cff_day = 'day';

            $cff_days = stripslashes( esc_attr( $options['cff_translate_days'] ) );
            if ( empty($cff_days) ) $cff_days = 'days';

            $cff_week = stripslashes( esc_attr( $options['cff_translate_week'] ) );
            if ( empty($cff_week) ) $cff_week = 'week';

            $cff_weeks = stripslashes( esc_attr( $options['cff_translate_weeks'] ) );
            if ( empty($cff_weeks) ) $cff_weeks = 'weeks';

            $cff_month = stripslashes( esc_attr( $options['cff_translate_month'] ) );
            if ( empty($cff_month) ) $cff_month = 'month';

            $cff_months = stripslashes( esc_attr( $options['cff_translate_months'] ) );
            if ( empty($cff_months) ) $cff_months = 'months';

            $cff_year = stripslashes( esc_attr( $options['cff_translate_year'] ) );
            if ( empty($cff_year) ) $cff_year = 'year';

            $cff_years = stripslashes( esc_attr( $options['cff_translate_years'] ) );
            if ( empty($cff_years) ) $cff_years = 'years';

            $cff_ago = stripslashes( esc_attr( $options['cff_translate_ago'] ) );
...
function cff_add_my_stylesheet() {
    ...
    wp_register_style( 'cff', plugins_url('/css/cff-style.css?15', __FILE__) );
...
function cff_scripts_method() {
    ...
    wp_register_script( 'cffscripts', plugins_url( '/js/cff-scripts.js?15' , __FILE__ ), array('jquery'), '1.8', true );
